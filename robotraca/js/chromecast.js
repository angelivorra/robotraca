...
function initializeCastApi() {
    try {
        ...
        // El Custom Receiver FBEBF31F aún no está completamente propagado
        const CUSTOM_RECEIVER_APP_ID = chrome.cast.media.DEFAULT_MEDIA_RECEIVER_APP_ID;
        // const CUSTOM_RECEIVER_APP_ID = 'FBEBF31F'; // Descomentar cuando esté listo
...
function setupCastIntegration() {
    ...
    window.pause = function() {
        if (isCasting && castSession) {
            ...
            if (mediaSession) {
                ...
                mobileLog.error('No media session available');
            }
        } else {
    ...
    window.nextSong = function() {
        if (isCasting) {
            // Si estamos en Cast, cambiar canción y enviar al Chromecast
            mobileLog.info('Next song -> Chromecast');
            // Cambiar índice localmente pero no reproducir
            currentSongIndex = (currentSongIndex + 1) % songs.length;
            updateSongDisplay();
            // Enviar nueva canción al Cast
            setTimeout(() => castCurrentSong(), 100);
        } else {
            // Si no, cambiar localmente
            originalNextSong();
        }
    };
